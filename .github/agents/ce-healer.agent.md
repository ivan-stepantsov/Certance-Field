...
```
scripts/lib/          - Core ESM library (Node.js ESM, "type": "module") - SOURCE OF TRUTH
scripts/compressors/  - Compression implementations - SOURCE OF TRUTH
vscode-extension/src/shared/  - GENERATED copy of scripts/lib + scripts/compressors
vscode-extension/src/ - Extension entry points (.cjs)
vscode-extension/test/ - Extension tests (node --test, .cjs)
```

**Critical:** `vscode-extension/src/shared/` is a generated copy of
`scripts/lib/` + `scripts/compressors/`. Never hand-edit `src/shared/`. Make all
changes in the root source, then regenerate with `npm run sync` (from
`vscode-extension/`). Sync is verified in CI by `npm run sync:check`, which fails
if the bundled copy drifts from source.
...
**CLASS A - Shared module drift**

Symptom: the `Verify shared modules in sync with root source` CI step (or
`npm run sync:check`) fails. The generated copy under
`vscode-extension/src/shared/` no longer matches `scripts/lib/` +
`scripts/compressors/`.

Root cause: either the root source changed without regenerating, or someone
hand-edited the generated `src/shared/` copy.

Fix protocol:
1. Make any real change in the root source (`scripts/lib/` / `scripts/compressors/`),
   never in `src/shared/`. If the drift came from a hand-edit to `src/shared/`,
   port that change into the corresponding root file first.
2. Regenerate the bundled copy:
   ```bash
   cd vscode-extension && npm run sync
   ```
3. Confirm in-sync and run the extension suite:
   ```bash
   cd vscode-extension && npm run sync:check && npm test
   ```
4. Commit the root change together with the regenerated `src/shared/`:
   ```
   fix(healer): regenerate shared modules from root source
   ```
...
3. Re-run the check to confirm clean:
   ...
   ```
4. If the fix touched `scripts/lib/` or `scripts/compressors/`, regenerate the bundled copy: `cd vscode-extension && npm run sync` (see Class A)
5. Commit:
   ```
...
|---|---|
| `npm run sync:check` (CI step) | `src/shared/` is byte-identical to `scripts/lib/` + `scripts/compressors/` -> see Class A |
...
shape) and between `scripts/lib/` and the generated `vscode-extension/src/shared/`.

Fix protocol:
1. Read the Codacy duplication finding - which files and line ranges are flagged
2. Determine whether the duplication is the generated `src/shared/` copy (expected)
   or accidental repetition within the root source
3. If accidental: extract the repeated logic into a shared utility in `scripts/lib/`
   and import it from both locations
4. If the duplication is the generated `src/shared/` copy: this is by design (it is
   produced by `npm run sync`) - exclude `vscode-extension/src/shared/` from Codacy
   via `.codacy.yml` `exclude_paths`, not an inline suppression
...
- Hand-edit the generated `vscode-extension/src/shared/` copy instead of the root
  source in `scripts/lib/` / `scripts/compressors/`
...
- Run `npm run sync` (then `npm run sync:check`) after any change to `scripts/lib/` or `scripts/compressors/`
...
Shared sync required: yes / no
  [if yes: ran `npm run sync` and committed regenerated src/shared/]
